...
import dao.cn.com.talkvip.bean.ImageSave;
import dao.cn.com.talkvip.utils.BitmapUtil;
import dao.cn.com.talkvip.utils.CommonUtil;
...
import dao.cn.com.talkvip.widget.ClipView;
import okhttp3.Call;
...
public class ClipHeaderActivity extends Activity implements OnTouchListener{
    ...
    private void generateUriAndReturn(){
        ...
        if (mSaveUri != null) {
            ...
            DebugFlags.logD("什么鬼"+mSaveUri);
            saveImage(getFileByUri(mSaveUri));

        }
    }


    public  void saveImage(File tempFile){
        String token= SPUtils.getString(ClipHeaderActivity.this,"token","");
        DebugFlags.logD(tempFile+"路径？");
       String[] names = tempFile.toString().split("\\/");
        DebugFlags.logD(names[names.length-1]+"路径？");
        if (tempFile!=null){

            OkHttpUtils.post()
                    .addFile("userfile",  names[names.length-1], tempFile)
                    ...
                    .execute(new StringCallback() {
                        ...
                        public void onResponse(String response, int id) {
                            DebugFlags.logD("保存相片"+response);
                      ImageSave is=      JSON.parseObject(response, ImageSave.class);
                            if ("success".equals(is.getResult())){
